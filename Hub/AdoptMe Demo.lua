    ...
    ["sleepy"] = function(c)
        while c.Parent == Player.PlayerGui.AilmentsMonitorApp.Ailments do
            spawn(function()
                ...
                wait(1)
                local SueloTemporalNuevo = Instance.new("Part", workspace)
                SueloTemporalNuevo.Name = "SueloTemporal"
                SueloTemporalNuevo.Size = Vector3.new(100, 0, 100)
                SueloTemporalNuevo.Anchored = true
                SueloTemporalNuevo.CFrame = Player.Character.HumanoidRootPart.CFrame * CFrame.new(Vector3.new(0,-5,0))
                if SueloTemporal ~= nil then
                    SueloTemporal:Destroy()
                end
                SueloTemporal = SueloTemporalNuevo
                wait(1)
                GoToHome()
                wait(1)
...
function Type(Name)
    game:GetService("ReplicatedStorage").API:FindFirstChild("AdoptAPI/HoldBaby"):FireServer(Pet)
    wait(1)
    SueloTemporal = Instance.new("Part", workspace)
...
local SettingsSection = AdoptMe:CreateSection("Settings")
